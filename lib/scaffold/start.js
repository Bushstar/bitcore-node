...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for feathercoind is necessary for this upgrade with the "reindex=1" feathercoin.conf option. \n' +
                 'There are changes necessary in both feathercoin.conf and feathercore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/feathercoind')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './feathercore-node.json');
